...
// color map as the points of the color gradient, should be at least two and no more than resolution
var colorMap = [
	[127, 0, 255], 
	[255, 255, 0]
];
...
function createHexColorMap(resolution) {
	resolution = parseInt(resolution+1);
	var hexColors = [];
	var spread = colorMap.length;
	if(spread == 1) {
		var hex = rgb2hex(colorMap[0]);
		...
		for(var i = 0; i < resolution; i++) {
			hexColors.push(rgb2hex(colorMap[i]));
		}
	} else {
		// convert to HSV first
		var hsvColors = [];
		for(var i = 0; i < spread; i++) {
			hsvColors.push(rgb2hsv(colorMap[i]));
		}
		...
		for(var i = 0; i < resolution; i++) {
			...
			hexColors.push(rgb2hex(hsv2rgb(hsv)));
		}
	}
	return hexColors;
}